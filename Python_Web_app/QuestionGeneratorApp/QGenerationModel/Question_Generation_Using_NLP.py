...
class QuestionGenerator:
    def generate(self, path_to_file):
        ...
        # test_set = pd.read_csv('Train_and_test_data/test.txt', sep=' ', names=['words','Brill','tag'],
        # index_col=False).drop(['Brill','tag'],1) test_set = test_set.values.tolist() ct.tag_sents(test_set)
        ...
        if os.path.isfile('QuestionGeneratorApp/QGenerationModel/LSTM_Model.baz') == False:
            ...
            lstm_model_file = 'QuestionGeneratorApp/QGenerationModel/LSTM_Model.baz'
            torch.save(model.state_dict(), lstm_model_file)
        else:
            model.load_state_dict(torch.load('QuestionGeneratorApp/QGenerationModel/LSTM_Model.baz'))
        ...
        questions_generated = generate_questions(model, refferingDataFrame)
